...
double calculateSpeed(double stx, double sty, double edx, double edy, int seconds) {
    int index = 0;
    try {
        double distance = scale[index] *
                          norm(matrixSubtraction(projector(index, edx, edy),
                                                 projector(index, stx, sty)));

        return distance * 3.6 / seconds;
    } 
	catch (exception& err) {
        return 0;
    }
